defmodule CatapultWeb.BoardLiveTest do
  ...
  test "the board renders every declared lane, in sequence order", %{conn: conn} do
  ...
  test "a ticket resting at a gate renders as a card in that gate's lane, linking to document-review",
       %{conn: conn} do
    project_id = "board-#{System.unique_integer([:positive])}"
