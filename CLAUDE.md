...
- **A rule stated in more than one place is amended in every place,
  ...
  on ORC-151 each corrected one statement and left its siblings:
  §15.1's mapping against §15.2's worked example five times over
  (`merge`, the `reconcile` count, `checks`, the child lifecycle,
  `pending`), then §13's `critique`-adjacency amendment against
  §15.5's own statement of that rule and §15.9's restatement of it —
  that last pair written by one pass, in one document, minutes apart.
  Grep the rule's own words before changing it: the second statement
  is rarely next to the first, and a worked example left behind by an
  amended rule is a load error nobody runs until dev.
